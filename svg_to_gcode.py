...
def parse_svg(file_path):
    ...
    red_paths = [path for path, attr in zip(paths, attributes)]
    return red_paths

def path_to_gcode(path, scale=1.0, power=255, bezier_steps=100):
    ...
    for i, segment in enumerate(path):
        if isinstance(segment, Line) or isinstance(segment, CubicBezier):
...
def svg_to_gcode(file_path, scale=1.0, power=255, pass_nbr=1, speed=90):
    ...
    for _ in range(pass_nbr):
        for path in red_paths:
            gcode += path_to_gcode(path, scale=scale, power=power)
...
def main():
    input_svg = 'wall1.svg'
    output_gcode = 'wall1.gcode'
    scale = 1.0
    power = 255
    pass_nbr = 120
    speed = 900

    gcode = svg_to_gcode(input_svg, scale=scale, power=power, pass_nbr=pass_nbr, speed=speed)
